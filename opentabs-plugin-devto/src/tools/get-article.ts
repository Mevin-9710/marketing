...
import { z } from 'zod';
import { fetchArticle, fetchArticleComments, type DEVComment } from '../devto-api.js';
...
export const getArticle = defineTool({
  ...
  async handle(params) {
    ...
    const flattenComments = (items: DEVComment[]): Array<{ body: string; author: string; createdAt: string }> =>
      items.flatMap(c => [
