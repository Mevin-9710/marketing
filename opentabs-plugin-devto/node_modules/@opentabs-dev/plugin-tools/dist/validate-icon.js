 ...
 */
const wcagRelativeLuminance = (red, green, blue) => {
    const linearize = (c) => {
        const srgb = c / 255;
...
const parseRgbComponent = (s) => (s.endsWith('%') ? Math.round(parseFloat(s) * 2.55) : parseInt(s, 10));
/**
...
const parseColor = (value) => {
    ...
    if (modernHslMatch) {
        return hslToRgb(parseFloat(modernHslMatch[1] ?? '0'), parseFloat(modernHslMatch[2] ?? '0'), parseFloat(modernHslMatch[3] ?? '0'));
    }
    // Named colors
...
const convertColorToGray = (value) => {
    ...
    const modernRgbConvertMatch = lowerValue.match(/^rgba?\(\s*([\d.]+%?)\s+([\d.]+%?)\s+([\d.]+%?)(?:\s*\/\s*([\d.]+%?))?\s*\)$/);
...
const generateInactiveIcon = (svgContent) => {
    ...
    result = result.replace(attrPattern, (_match, prefix, quote, value) => {
    ...
    const styleBlockPattern = /(<style[^>]*>)([\s\S]*?)(<\/style>)/gi;
    result = result.replace(styleBlockPattern, (_fullMatch, openTag, cssContent, closeTag) => {
        let newCss = cssContent;
        for (const attr of COLOR_ATTRS) {
            const cssPropPattern = new RegExp(`(${attr.replace('-', '\\-')}\\s*:\\s*)([^;}"']+)`, 'gi');
            newCss = newCss.replace(cssPropPattern, (_m, propPrefix, propValue) => {
                const converted = convertColorToGray(propValue);
                return `${propPrefix}${converted}`;
            });
        }
 ...
 */
const convertColorForDark = (value) => {
    const trimmedValue = value.trim();
    const lowerValue = trimmedValue.toLowerCase();
    if (PASSTHROUGH_VALUES.has(lowerValue))
        return trimmedValue;
 ...
 */
const generateDarkIcon = (svgContent) => {
    let result = svgContent;
    // Convert attribute values: (fill|stroke|stop-color|flood-color)="value" or ='value'
    const attrPattern = new RegExp(`((?:${COLOR_ATTRS.join('|')})\\s*=\\s*)(["'])([^"']*)(\\2)`, 'gi');
    result = result.replace(attrPattern, (_match, prefix, quote, value) => {
...
const namespaceSvgIds = (svgContent, prefix) => {
    const ids = Array.from(svgContent.matchAll(/\bid\s*=\s*"([^"]+)"/g), m => m[1]);
    if (ids.length === 0)
        return svgContent;
    let result = svgContent;
    for (const id of ids) {
...
export { DARK_BG_LUMINANCE, generateDarkIcon, generateInactiveIcon, MAX_ICON_SIZE, MIN_ICON_CONTRAST, MIN_INACTIVE_GRAY, namespaceSvgIds, validateIconSvg, validateInactiveIconColors, };
