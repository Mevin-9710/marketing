 ...
 */
const extractShortName = (packageName) => {
...
const generateReadme = (meta, tools, configSchema) => {
    ...
    lines.push('');
    lines.push(`${meta.description} — gives AI agents access to ${meta.displayName} through your authenticated browser session.`);
    lines.push('');
    // Install
    lines.push('## Install');
    lines.push('');
    ...
    lines.push('');
    lines.push(`This plugin runs inside your ${meta.displayName} tab through the [OpenTabs](https://opentabs.dev) Chrome extension. It uses your existing browser session — no API tokens or OAuth apps required. All operations happen as you, with your permissions.`);
    lines.push('');
    // License
    lines.push('## License');
    lines.push('');
...
const handleReadme = async (options, projectDir = process.cwd()) => {
    // Read dist/tools.json
    const toolsJsonPath = join(projectDir, 'dist', TOOLS_FILENAME);
    if (!(await access(toolsJsonPath).then(() => true, () => false))) {
    ...
    let configSchema;
    try {
        const parsed = JSON.parse(await readFile(toolsJsonPath, 'utf-8'));
        ...
        tools = obj.tools;
        configSchema = (obj.configSchema ?? undefined);
    }
    catch {
    ...
    const pkgJsonRaw = JSON.parse(await readFile(pkgJsonPath, 'utf-8'));
...
const registerReadmeCommand = (program) => {
    program
        .command('readme')
...
Examples:
  ...
  $ opentabs-plugin readme --check`)
        .action((options) => handleReadme(options));
};
export { classifyTool, extractDomain, extractShortName, generateReadme, groupTools, handleReadme, registerReadmeCommand, };
