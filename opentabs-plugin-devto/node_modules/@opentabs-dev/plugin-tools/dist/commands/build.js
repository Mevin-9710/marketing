...
import { ADAPTER_FILENAME, ADAPTER_SOURCE_MAP_FILENAME, atomicWrite, DEFAULT_HOST, DEFAULT_PORT, getConfigDir, getConfigPath, PRE_SCRIPT_FILENAME, parsePluginPackageJson, pluginNameFromPackage, TOOLS_FILENAME, toErrorMessage, } from '@opentabs-dev/shared';
import { build as esbuild } from 'esbuild';
import pc from 'picocolors';
import { z } from 'zod';
import { generateDarkIcon, generateInactiveIcon, namespaceSvgIds, validateIconSvg, validateInactiveIconColors, } from '../validate-icon.js';
...
const atomicWriteConfig = (configPath, content) => atomicWrite(configPath, content, 0o600);
 ...
 * the lock directory.
 */
const acquireConfigLock = async (configPath) => {
    ...
    for (let attempt = 0; attempt < CONFIG_LOCK_MAX_RETRIES; attempt++) {
        ...
        try {
            ...
            if (ageMs > STALE_LOCK_THRESHOLD_MS) {
                ...
                if (!holderAlive) {
                    console.warn(pc.yellow(`Warning: Stale config lock detected (${Math.round(ageMs / 1_000)}s old). Removing and retrying.`));
                    try {
                        rmSync(lockDir, { recursive: true });
                    }
                    catch {
        ...
        await new Promise(r => setTimeout(r, CONFIG_LOCK_RETRY_DELAY_MS));
    }
 ...
 */
const resolvePluginPathForComparison = (storedPath, configDir) => {
    if (isAbsolute(storedPath))
        return storedPath;
    if (storedPath.startsWith('~/'))
        return resolve(homedir(), storedPath.slice(2));
 ...
 */
const toPortablePath = (absolutePath) => {
    const home = homedir();
 ...
 */
const registerInConfig = async (projectDir) => {
    if (process.env.OPENTABS_SKIP_REGISTER === '1')
        return false;
    ...
    try {
        ...
        if (!Array.isArray(config.localPlugins)) {
            ...
            config.localPlugins = config.localPlugins.filter((p) => typeof p === 'string');
        }
        const plugins = config.localPlugins;
        const absolutePath = resolve(projectDir);
        const configDir = dirname(configPath);
        // Check for duplicates by comparing resolved absolute paths
        const alreadyRegistered = plugins.some(existing => resolvePluginPathForComparison(existing, configDir) === absolutePath);
...
const notifyServer = async () => {
    if (process.env.OPENTABS_SKIP_NOTIFY === '1')
        return;
    const authJsonPath = join(getConfigDir(), 'extension', 'auth.json');
    if (!(await access(authJsonPath).then(() => true, () => false)))
        return;
    let secret;
    try {
        const parsed = JSON.parse(await readFile(authJsonPath, 'utf-8'));
        if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed)) {
            const record = parsed;
...
const validatePackageJson = (pkgJson, projectDir) => {
    ...
    for (const pattern of result.value.opentabs.excludePatterns ?? []) {
        const patternError = validateUrlPattern(pattern);
        if (patternError)
            throw new Error(`Invalid exclude pattern: ${patternError}`);
    }
    return result.value;
};
const validatePlugin = (plugin) => {
    ...
    // URL patterns
    const hasRequiredUrlSetting = plugin.configSchema && Object.values(plugin.configSchema).some(s => s.type === 'url' && s.required);
    ...
    if (plugin.excludePatterns) {
        for (const pattern of plugin.excludePatterns) {
            const patternError = validateUrlPattern(pattern);
            if (patternError)
                errors.push(`Invalid exclude pattern: ${patternError}`);
        }
    }
    // Homepage (optional)
    ...
    if (plugin.tools.length === 0) {
        ...
        for (const tool of plugin.tools) {
            ...
            if (tool.displayName !== undefined && tool.displayName.length === 0)
                errors.push(`Tool "${tool.name || '(unnamed)'}" has an empty displayName — either omit it for auto-derivation or provide a non-empty value`);
 ...
 */
const jsStringLiteral = (value) => JSON.stringify(value).replace(/</g, '\\u003c').replace(/>/g, '\\u003e');
const minifySvg = (svg) => {
 ...
 */
const readAndValidateIcons = async (projectDir, pluginName) => {
    ...
    if (!hasIcon && hasInactive) {
        throw new Error('icon-inactive.svg requires icon.svg to also be present. Add an icon.svg or remove icon-inactive.svg.');
    }
    ...
    if (!hasDark && hasDarkInactive) {
        throw new Error('icon-dark-inactive.svg requires icon-dark.svg to also be present. Add an icon-dark.svg or remove icon-dark-inactive.svg.');
    }
    ...
    const parts = [];
    if (hasInactive)
        parts.push('icon-inactive.svg');
    if (hasDark)
        parts.push('icon-dark.svg');
    if (hasDarkInactive)
        parts.push('icon-dark-inactive.svg');
    const autoGenerated = [];
    if (!hasInactive)
        autoGenerated.push('inactive');
    if (!hasDark)
        autoGenerated.push('dark');
    if (!hasDarkInactive)
        autoGenerated.push('dark-inactive');
    console.log(pc.dim(`Plugin icon: icon.svg${parts.length > 0 ? ` + ${parts.join(' + ')}` : ''} found` +
 ...
 */
const deriveDisplayName = (name) => name
    .split('_')
    .map(w => {
...
const generateToolsManifest = (plugin) => plugin.tools.map(tool => {
 ...
 */
const resolveSdkVersion = async (projectDir) => {
    const sdkPkgPath = join(projectDir, 'node_modules', '@opentabs-dev', 'plugin-sdk', 'package.json');
    if (!(await access(sdkPkgPath).then(() => true, () => false))) {
...
const generateManifest = (plugin, sdkVersion, icons, preScript) => ({
    sdkVersion,
 ...
 */
const bundlePreScript = async (preScriptEntry, outDir, pluginName) => {
...
const runBuild = async (projectDir) => {
    ...
    if (!(await access(pkgJsonPath).then(() => true, () => false))) {
    ...
    if (!(await access(entryPoint).then(() => true, () => false))) {
        const sourceExists = await access(sourceEntry).then(() => true, () => false);
        if (!sourceExists) {
            throw new Error(`Neither compiled output (${entryPoint}) nor source (${sourceEntry}) found. Is this a plugin directory?`);
        }
        ...
        if (tscResult.error) {
            if (tscResult.error.code === 'ENOENT') {
    ...
    try {
        ...
        if (!ready) {
            console.warn(pc.yellow('Warning: isReady() returned false — the plugin will report as "unavailable" in the browser. Update isReady() with a real authentication check (e.g., check for session cookies, a DOM element, or a page global).'));
        }
    }
    catch {
    ...
    if (plugin.name !== derivedName) {
        console.warn(pc.yellow(`Warning: plugin class name "${plugin.name}" does not match the name derived from package.json ("${derivedName}"). The adapter will be registered as "${derivedName}". Update the plugin class name to avoid confusion.`));
    }
    ...
    if (await access(sourceMapPath).then(() => true, () => false)) {
    ...
    if (pkgJson.opentabs.preScript) {
        ...
        const sourceExists = await access(preScriptSourcePath).then(() => true, () => false);
        if (!sourceExists) {
            throw new Error(`Declared preScript source not found: ${preScriptSourcePath}. Check the "opentabs.preScript" field in package.json.`);
        }
    ...
    if (icons?.iconSvg) {
        const rawPkg = JSON.parse(await readFile(pkgJsonPath, 'utf-8'));
        const opentabs = rawPkg.opentabs;
