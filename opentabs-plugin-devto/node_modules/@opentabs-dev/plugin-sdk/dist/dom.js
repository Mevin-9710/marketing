 ...
 */
const ATTRIBUTE_PSEUDO_RE = /:(checked|disabled|enabled|required|optional|read-only|read-write|default|valid|invalid|in-range|out-of-range|placeholder-shown)/;
/**
 ...
 */
const needsAttributeObservation = (selector) => selector.includes('[') || selector.includes('.') || ATTRIBUTE_PSEUDO_RE.test(selector);
/**
...
export const waitForSelectorRemoval = (selector, opts) => {
    const timeout = opts?.timeout ?? 10_000;
    const signal = opts?.signal;
    const abortReason = () => signal?.reason instanceof Error ? signal.reason : new Error('waitForSelectorRemoval: aborted');
    if (signal?.aborted)
        return Promise.reject(abortReason());
    return new Promise((resolve, reject) => {
        let hasElement;
...
export const getMetaContent = (name) => {
    if (!name)
        return null;
    try {
        const meta = document.querySelector(`meta[name="${CSS.escape(name)}"]`);
