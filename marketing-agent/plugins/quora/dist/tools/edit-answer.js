...
export const editAnswer = defineTool({
    ...
    async handle(params) {
        ...
        const editButton = (Array.from(document.querySelectorAll('button')).find(b => /edit/i.test(b.getAttribute('aria-label') || b.textContent || '')) || document.querySelector('.puppeteer_test_edit_answer_button, [data-testid="edit_answer"]'));
        if (!editButton) {
        ...
        const saveButton = (Array.from(document.querySelectorAll('button')).find(b => /save|submit/i.test(b.textContent || '') || b.type === 'submit'));
        if (saveButton) {
            saveButton.click();
        }
