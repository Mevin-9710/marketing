    ...
    async handle(params) {
        ...
        if (!params.question_url || window.location.href.includes(params.question_url)) {
            ...
            if (editor) {
                ...
                const submitButton = (Array.from(document.querySelectorAll('button')).find(b => b.type === 'submit' || /submit|post|add.?answer/i.test(b.textContent || '')) || document.querySelector('[data-testid="submit_answer_button"], .puppeteer_test_submit_answer'));
                if (submitButton) {
                    submitButton.click();
        ...
        const result = await postJSON('/answer/logged_in_answer_creation', {
...
//# sourceMappingURL=post-answer.js.map