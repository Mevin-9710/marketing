...
export const getUserProfile = defineTool({
    ...
    async handle(params) {
        ...
        const profileImage = doc
            ? root.querySelector('[data-testid="profile_photo"] img')
                || Array.from(root.querySelectorAll('img')).find(img => (img.alt || '').toLowerCase().includes('profile') || (img.className || '').toLowerCase().includes('profile'))
            : (document.querySelector('[data-testid="profile_photo"] img')
                || Array.from(document.querySelectorAll('img')).find(img => (img.alt || '').toLowerCase().includes('profile') || (img.className || '').toLowerCase().includes('profile')));
