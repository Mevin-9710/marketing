...
export const createQuestion = defineTool({
  ...
  async handle(params) {
    ...
    const addQuestionBtn = (document.querySelector('[data-testid="add_question_button"], a[href*="/add"]')
      || Array.from(document.querySelectorAll('button')).find(b => /add.?question/i.test(b.textContent || ''))) as HTMLElement | null;
    if (addQuestionBtn) {
      addQuestionBtn.click();
      await new Promise(resolve => setTimeout(resolve, 1000));

      const titleInput = document.querySelector('[data-testid="question_title_input"], input[placeholder*="question" i], textarea[placeholder*="question" i]') as HTMLInputElement | null;
      if (titleInput) {
      ...
      if (params.details) {
        const detailsInput = document.querySelector('[contenteditable="true"], [data-testid="question_details_input"], textarea') as HTMLElement | null;
        if (detailsInput) {
      ...
      const submitBtn = (Array.from(document.querySelectorAll('button')).find(b =>
        b.type === 'submit' || /add.?question|post|ask/i.test(b.textContent || '')
      )) as HTMLElement | null;
      if (submitBtn) {
        submitBtn.click();
