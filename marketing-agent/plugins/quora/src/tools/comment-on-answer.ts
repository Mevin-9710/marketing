...
export const commentOnAnswer = defineTool({
  ...
  async handle(params) {
    ...
    const commentBox = (document.querySelector('[data-testid="comment_input"]')
      || Array.from(document.querySelectorAll('[contenteditable="true"], textarea')).find(el =>
        /comment/i.test(el.getAttribute('placeholder') || el.getAttribute('data-placeholder') || '')
      )) as HTMLElement | null;

    if (!commentBox) {
      const commentTrigger = (document.querySelector('[data-testid="comment_button"]')
        || Array.from(document.querySelectorAll('button')).find(b => /comment/i.test(b.textContent || ''))) as HTMLElement | null;
      if (commentTrigger) {
    ...
    const textarea = document.querySelector('[data-testid="comment_input"], textarea, [contenteditable="true"]') as HTMLElement | null;
    if (!textarea) {
    ...
    const submitBtn = (Array.from(document.querySelectorAll('button')).find(b =>
      b.type === 'submit' || /add|comment|post/i.test(b.textContent || '')
    )) as HTMLElement | null;
    if (submitBtn) {
      submitBtn.click();
    }
