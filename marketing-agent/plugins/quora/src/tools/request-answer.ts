...
export const requestAnswer = defineTool({
  ...
  async handle(params) {
    ...
    const requestBtn = (Array.from(document.querySelectorAll('button')).find(b =>
      /request/i.test(b.textContent || '')
    ) || document.querySelector('[data-testid="request_answer_button"]')) as HTMLElement | null;
    ...
    const userInput = document.querySelector('input[placeholder*="name"], [data-testid="answer_request_input"]') as HTMLElement | null;
    if (userInput) {
    ...
    if (params.message) {
      const msgInput = document.querySelector('textarea, [contenteditable="true"]:not([data-testid="answer_request_input"])') as HTMLElement | null;
      if (msgInput) {
    ...
    const sendBtn = (Array.from(document.querySelectorAll('button')).find(b =>
      /send|request|invite/i.test(b.textContent || '')
    )) as HTMLElement | null;
    if (sendBtn) {
      sendBtn.click();
