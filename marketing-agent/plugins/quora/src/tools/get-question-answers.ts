...
export const getQuestionAnswers = defineTool({
  ...
  async handle(params) {
    ...
    let questionInfo: { id: string; title: string; url: string } | null = null;
    let rawAnswers: Array<{ id: string; author: string; content: string; upvoteCount: number; timestamp?: string }> = [];
        ...
        for (const key of Object.keys(apolloState)) {
          ...
          if (entry.__typename === 'Question' && entry.id && !questionInfo) {
            questionInfo = {
              ...
              url: String((entry.url as string) || targetUrl),
            };
          }
          if (entry.__typename === 'Answer' || entry.__typename === 'AnswerWithDraft') {
            let authorName = '';
            const authorRef = entry.author as { __ref?: string } | undefined;
            if (authorRef?.__ref) {
              const authorData = apolloState[authorRef.__ref] as Record<string, unknown> | undefined;
              if (authorData) {
            ...
            rawAnswers.push({
              id: String(entry.id || ''),
              author: authorName,
              content: String((entry.content || entry.text || entry.answer || '') as string).substring(0, 500),
              upvoteCount: Number((entry.upvoteCount || entry.votes || 0) as number),
