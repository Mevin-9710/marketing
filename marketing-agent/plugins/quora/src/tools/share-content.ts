...
export const shareContent = defineTool({
  ...
  async handle(params) {
    ...
    const shareButton = document.querySelector('[data-testid="share_button"], button:has(svg.fa-share), .puppeteer_test_share') as HTMLElement | null;
    if (shareButton) {
      shareButton.click();
      ...
      if (params.platform === 'copy') {
        ...
        /copy/i.test(b.textContent || b.getAttribute('aria-label') || '')
      )) as HTMLElement | null;
        if (copyBtn) {
          copyBtn.click();
      ...
      const linkInput = document.querySelector('input[readonly], input[type="url"]') as HTMLInputElement | null;
      if (linkInput) {
