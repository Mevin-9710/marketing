...
export const upvoteTool = defineTool({
    ...
    async handle(params) {
        ...
        const upvoteBtn = document.querySelector('button[class*="vote"], [data-testid="upvote"], [aria-label*="upvote"], button:has(svg), [class*="VoteButton"]');
        if (upvoteBtn) {
            upvoteBtn.click();
        ...
        const voteBtn = allButtons.find(b => /vote|upvote|\b214\b|upvote/i.test(b.textContent || ''));
        if (voteBtn) {
            voteBtn.click();
            return { success: true, message: 'Tool upvoted successfully' };
        }
...
//# sourceMappingURL=upvote-tool.js.map