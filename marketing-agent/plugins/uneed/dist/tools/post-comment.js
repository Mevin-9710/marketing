...
export const postCommentTool = defineTool({
    ...
    async handle(params) {
        ...
        const commentInput = document.querySelector('textarea[class*="comment"], textarea, [contenteditable="true"]');
        if (!commentInput) {
        ...
        const submitBtn = Array.from(document.querySelectorAll('button')).find(b => /comment|reply|submit|post|send/i.test(b.textContent || ''));
        if (submitBtn) {
            submitBtn.click();
