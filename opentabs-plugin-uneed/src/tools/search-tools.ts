...
export const searchTools = defineTool({
  ...
  async handle(params) {
    ...
    const results: Array<{ name: string; description?: string; url?: string; voteCount?: number; tags?: string[] }> = [];
    ...
    cards.forEach(el => {
      ...
      const link = (el as HTMLAnchorElement).href || (el.querySelector('a') as HTMLAnchorElement)?.href || '';
